...
extern "C" void dumpDexFileByExecute(ArtMethod * artmethod) REQUIRES_SHARED(Locks::mutator_lock_) {
    ...
    if (szProcName[0]) {
        ...
        else
        {
          ...
          if (dexfilefp > 0) 
          {
            (void)write(dexfilefp, (void *) begin_,size_);
            fsync(dexfilefp);
            close(dexfilefp);
          }
        }
...
extern "C" void dumpArtMethod(ArtMethod * artmethod) REQUIRES_SHARED(Locks::mutator_lock_) 
{
        ...
        if (szProcName[0]) 
        {
          ...
          if (LIKELY(code_item != nullptr)) 
          {
              ...
              {
                  const uint8_t *handler_data = (const uint8_t *) (DexFile::GetTryItems(*code_item,
              ...
              uint32_t method_idx = artmethod->GetMethodIndex();//artmethod->get_method_idx();
              ...
              if (fp2 > 0) 
              {
                ...
                while (dexfilepath[contentlength]!= 0)
                    contentlength++;
                (void)write(fp2, (void *) dexfilepath,contentlength);
                long outlen = 0;
                char *base64result =
